...
class Settings(BaseSettings):
    ...
    # ── Webshare Proxy (For Metadata) ──
    webshare_proxy_username: str = ""
    webshare_proxy_password: str = ""

    # ── RapidAPI (For Transcript) ──
    rapidapi_key: str = ""
...
# ────── GLOBAL PROXY SETUP (For Google API client only) ──────
# We set this globally so googleapiclient picks it up automatically.
# This is required for fetch_metadata to work on Railway/Cloud.
...
# so we use the Groq SDK directly with a trust_env=False httpx client.
groq_vision_client = Groq(
...
LANGUAGE_CODE_MAP = {
    "english": "en", "en": "en", "urdu": "ur", "hindi": "hi",
...
def fetch_metadata(video_id: str) -> Optional[dict]:
    ...
    try:
        print("[FETCH_METADATA] Fetching via Google API...")
        youtube = build("youtube", "v3", developerKey=settings.youtube_api_key)
        request = youtube.videos().list(part="snippet", id=video_id)
        response = request.execute()

        if not response.get("items"):
            print(f"[FETCH_METADATA] No items found for video_id={video_id}")
            return None
...
def fetch_transcript(video_id: str, language: str = "English") -> Optional[str]:
    ...
    try:
        ...
        if not transcript_text:
            print("[FETCH_TRANSCRIPT] ⚠️  Parsed transcript is empty.")
            return None

        print(f"[FETCH_TRANSCRIPT] ✅ Success — {len(transcript_text)} chars.")
        return transcript_text.strip()

    except httpx.HTTPStatusError as e:
        print(f"[FETCH_TRANSCRIPT][HTTP ERROR] {e.response.status_code}: {e.response.text[:300]}")
        return None
    except Exception as e:
                ...
                with httpx.Client(timeout=10, trust_env=False) as client:
                    ...
                    if resp.status_code == 200:
                        ...
                        if content_length and int(content_length) > 2000:
                            ...
                            print(f"[FETCH_THUMBNAIL][SKIP] {candidate} — placeholder")
                    else:
...
def validate_thumbnail_url(thumbnail_url: str) -> Optional[str]:
    ...
    try:
        with httpx.Client(timeout=10, trust_env=False) as client:
            resp = client.head(thumbnail_url)
            if resp.status_code == 200:
                content_length = resp.headers.get("content-length")
                if content_length and int(content_length) < 2000:
                    print(f"[THUMBNAIL_VALIDATE][SKIP] {thumbnail_url} — placeholder ({content_length} bytes)")
                    return None
                print(f"[THUMBNAIL_VALIDATE][OK] {thumbnail_url}")
                return thumbnail_url
            else:
                print(f"[THUMBNAIL_VALIDATE][SKIP] {thumbnail_url} — HTTP {resp.status_code}")
    except Exception as e:
...
def generate_single_image(prompt: str) -> Dict:
    ...
    Uses trust_env=False to bypass the global Webshare proxy —
    both Cloudflare Workers AI and ImgBB are publicly accessible.
    """
    try:
        # ✅ FIX: trust_env=False prevents 407 Proxy Authentication Required
...
def _invoke_vision(image_url: str) -> str:
    """Analyze a thumbnail image using Groq Vision (direct SDK).

    ✅ FIX: Uses groq.Groq SDK directly with httpx.Client(trust_env=False)
    instead of ChatGroq which doesn't reliably bypass the proxy.
    """
...
def thumbnail_analyzer_node(state: MetadataState) -> MetadataState:
    """Analyze thumbnail via Groq Vision and produce a regeneration prompt."""
    ...
    print(f"[VISION] Analyzing thumbnail: {thumbnail_url}")
    try:
